...
code: for instance, having to decipher obscure output such as GNU make
reporting a g++ error because the wrong version of libboost-dev is installed.
...
processing; for example, this might be --input, --fastq or the position in the
input arguments. The output BAM file generated by a short read aligner may be
created in different locations, or the output may not be stored in BAM at all.
Across all available short read aligners this leads to a multitude of different
ways of doing the same task each time: take a list of sequence reads and return
a description of how they map to a reference genome.
...
shows a command line interface to bioboxes that allows the reader to test out a
biobox.
